...
class AgentCore(
    ...
    fun run(userRequest: String, onComplete: (AgentResult) -> Unit) {
        ...
        stuckDetectors[taskId] = StuckDetector(MAX_STEPS, MAX_TASK_WALL_MS)
        ...
        when (route) {
            ...
            is RouteDecision.ModelTool -> generateModel(
                taskId,
            ...
            is RouteDecision.Agent -> generateModel(
                taskId,
                promptBuilder.initial(request),
                promptBuilder.exposedToolCount(ToolGroup.entries.toSet()),
            ) { modelOutput ->
                if (taskSessionStore.isActive(taskId)) {
                    handleModelOutput(taskId, request, modelOutput, step = 1, onComplete = onComplete)
                }
    ...
    private fun completeExecution(
        ...
        if (result.requiresFollowUp && userRequest.isNotBlank() && step >= maxSteps) {
            stuckDetectors[taskId]?.stepBudgetDecision(step)
            finishFinal(taskId, result.message, failed = true, onComplete)
            return
        }
        ...
        taskSessionStore.transition(taskId, TaskState.WAITING_FOR_MODEL)
        generateModel(taskId, promptBuilder.afterTool(userRequest, result), exposedToolCount = 0) { output ->
            if (taskSessionStore.isActive(taskId)) {
